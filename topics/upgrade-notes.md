...
### Known Issues
{id="known-issues-2024-12-1"}

* OpenSSH format SSH keys generated by Devolutions Remote Desktop Manager cannot be used within TeamCity. As a workaround, these keys could be fixed by importing them in `puttygen` and exporting them back in OpenSSH format.  See this YouTrack ticket for more information: [TW-91528](https://youtrack.jetbrains.com/issue/TW-91528).
...
* Linux AMD64 [server image](https://hub.docker.com/r/jetbrains/teamcity-server/tags) fails to set the locale, causing special characters to render incorrectly. If these characters appear in branch, VCS root, project, or other names, it triggers various TeamCity error reports.
